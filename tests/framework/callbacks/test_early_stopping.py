...
class EarlyStoppingTest(unittest.TestCase):
    ...
    @patch("torchtnt.framework.callbacks.early_stopping.EarlyStopping._should_stop")
    def test_interval(self, _should_stop: MagicMock) -> None:
        early_stop_checker = EarlyStopChecker(
            mode="min",
            patience=2,
            min_delta=0.0,
        ...
        for interval in ("step", "epoch"):
            esc = EarlyStopping(
                monitored_attr="train_loss",
                early_stop_checker=early_stop_checker,
                # pyre-ignore Incompatible parameter type [6]: expected `Union[typing_extensions.Literal['epoch'], typing_extensions.Literal['step']] but got `str``
                interval=interval,
            )

            esc.on_train_step_end(state, unit)
            if interval == "step":
                _should_stop.assert_called_once()
            else:
                _should_stop.assert_not_called()
            _should_stop.reset_mock()
    ...
    @patch("torchtnt.framework.callbacks.early_stopping.EarlyStopping._should_stop")
    def test_interval_freq(self, _should_stop: MagicMock) -> None:
        early_stop_checker = EarlyStopChecker(
            mode="min",
            patience=2,
            min_delta=0.0,
