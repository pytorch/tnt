...
class AppStateMixinTest(unittest.TestCase):
    ...
    def test_tracked_optimizers(self) -> None:
        ...
        class MyUnit(_AppStateMixin):
            def __init__(self):
                super().__init__()
                module_a = nn.Linear(1, 1)
                self.optimizer_c = torch.optim.SGD(module_a.parameters(), lr=0.1)
    ...
    def test_tracked_lr_schedulers(self) -> None:
        ...
        class MyUnit(_AppStateMixin):
            def __init__(self):
                super().__init__()
                module_a = nn.Linear(1, 1)
                optimizer_b = torch.optim.SGD(module_a.parameters(), lr=0.1)
                self.lr_scheduler_d = torch.optim.lr_scheduler.StepLR(
                    optimizer_b, step_size=30, gamma=0.1
                )
