...
def prepare_module(
    ...
    if strategy:
        if strategy == "ddp":
            ...
            module = DDP(module, device_ids=device_ids)
        elif strategy == "fsdp":
            # wrap module in FSDP
            module = FSDP(
                module,
...
class MyTrainUnit(TrainUnit):
    def __init__(
        ...
        self.metric = MulticlassAccuracy().to(device)
        self.log_frequency_steps = log_frequency_steps

        path = tempfile.mkdtemp()
        self.logger = TensorBoardLogger(path)
        ...
        self.engine = SGDEngine(self.module, self.optimizer, get_loss)

    def train_step(self, state: State, batch: Batch) -> None:
        loss, outputs = self.engine.step(state, batch)
        # update metrics & logs
        _, targets = batch
        self.metric.update(outputs, targets)
        progress = state.train_state.progress
        if progress.num_steps_completed % self.log_frequency_steps == 0:
            acc = self.metric.compute()
            self.logger.log("loss", loss, progress.num_steps_completed)
            self.logger.log("accuracy", acc, progress.num_steps_completed)
