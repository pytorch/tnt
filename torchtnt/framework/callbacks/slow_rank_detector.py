...
class SlowRankDetector(Callback):
    ...
    If a logger is passed, the differnce between the fastest rank and slowest rank is also reported.
    ...
    def _sync_times(self, epochs: int, steps: int) -> None:
        ...
        logger.info(
            f"""Time difference between fastest rank ({fastest_rank}: {time_on_fastest_rank} sec) and slowest rank ({slowest_rank}: {time_on_slowest_rank} sec) is {time_difference} seconds."""
        )
