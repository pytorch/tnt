...
class VisdomPlotLogger(BaseVisdomLogger):
    ...
    def log(self, *args, **kwargs):
        if self.win is not None and self.viz.win_exists(win=self.win, env=self.env):
            if len(args) != 2:
                raise ValueError(
                    "When logging to {}, must pass in x and y values (and optionally z).".format(
                        type(self)
                    )
                )
            x, y = args
...
class VisdomTextLogger(BaseVisdomLogger):
    ...
    def __init__(
        ...
        env=None,
        opts={},
        update_type=valid_update_types[0],
        port=8097,
        server="localhost",
    ):
        ...
        if update_type not in self.valid_update_types:
            raise ValueError(
                "update type '{}' not found. Must be one of {}".format(
    ...
    def _log_all(self, stats, log_fields, prefix=None, suffix=None, require_dict=False):
        ...
        for field_idx, field in enumerate(self.fields):
            ...
            for f in field:
                parent, stat = stat, stat[f]
            name, output = self._gather_outputs(
                field, log_fields, parent, stat, require_dict
            )
            if not output:
                continue
